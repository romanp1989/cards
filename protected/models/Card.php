...
class Card extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('series, number, create_date, expiration_date, used_date', 'required'),
			array('total, status', 'numerical', 'integerOnly'=>true),
			array('series, number', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, series, number, create_date, expiration_date, used_date, total, status', 'safe', 'on'=>'search'),
		);
	}
